...
object MethodTenantHostIdentifier {
  ...
  ): RoutingFactory.Identifier[Request] = MethodTenantHostIdentifier(prefix, baseDtab)
}
    ...
    case Version.Http10 =>
      Future.value(
        new UnidentifiedRequest(
          s"${Version.Http10} not supported by this identifier"
        )
      )
    case Version.Http11 =>
      req.host match {
        ...
        case _ =>
          Future.value(
            new UnidentifiedRequest(
              s"${Version.Http11} request missing Host header"
            )
          )
      }
  }
